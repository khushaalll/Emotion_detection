...
def train_model(model, model_name, fold, train_loader, val_loader, epochs=10, early_stopping_patience=5):
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        print(f'Fold: {fold} Epoch: {epoch + 1} \tTraining Loss: {train_loss:.6f} \tValidation Loss: {val_loss:.6f}')
        ...
        if val_loss < best_val_loss:
            ...
            model_save_path = f'{model_name}_fold_{fold}_best.pth'
...
def main():
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(train_data)):
        ...
        # Train the model
        train_model(model, "FacialCNN", fold, train_loader, val_loader)
